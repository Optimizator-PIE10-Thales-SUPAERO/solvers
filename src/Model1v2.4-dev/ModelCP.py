...
def SimpleSatProgram(model,dict_data,dict_non_visib,n_tasks,list_sats,list_antennes,filename,setOcc=-1,flag_nostore=False,flag_nooccurrence=False):
            ...
            if (sat_id,antenne_id) in dict_non_visib.keys():
                ...
                for (s,e) in list_intervals:
                    for rep_id in range(dict_max_repetive[task_id]):
                        ...
                        t1_bool = model.NewBoolVar("t1_"+str(task_id)+str(antenne_id)+str(s))
                        t2_bool = model.NewBoolVar("t2_"+str(task_id)+str(antenne_id)+str(s))
    ...
    # Constraint 5: time margin between interval used should be bounded by max lag (min lag is already be assured by other constraints)
    print("(not assured for now)Constraint5: Time margin between interval used should be bounded by max lag")
    """
    for task_id in range(n_tasks):
        ...
        for i in range(len(tmp_list_occ)):
            for j in range(i+1,len(tmp_list_occ)):
                ...
                model.Add(e1.start >= 0).OnlyEnforceIf(bool_var1)
                model.Add(e2.start >= 0).OnlyEnforceIf(bool_var2)
                # we have already assure that the difference between two intervals is larger than min time lag
                model.Add(e2.start - e1.end <= max_lag - min_lag).OnlyEnforceIf(bool_var3)
                # model.Add(e1.end >= e2.start - max_lag).OnlyEnforceIf(bool_var3)
                #model.Add(e2.end <= - min_lag).OnlyEnforceIf(bool_var4)
                model.Add(e1.start - e2.end <= max_lag - min_lag).OnlyEnforceIf(bool_var4)
    
                tmp_bool_list.append(bool_var1)
                tmp_bool_list.append(bool_var2)
                tmp_bool_list.append(bool_var3)
                tmp_bool_list.append(bool_var4)
        ...
        for task_id in range(n_tasks):
            ...
            for assign in assigned_intervals[task_id]:
                ...
                if(start != end):
                    ...
                    sol_line += '%-20s' %name
                    sol_tmp = '[%i,%i]' % (start, end-dict_min_lag[task_id])
                    # Add spaces to output align columns
                    sol_line += '%-15s\n' % sol_tmp
